...
async def connect(sid, environment):
    ...
    async with httpx.AsyncClient() as client:
        response = await client.post(endpoint_url, json={'sid': sid}, timeout=30.)
...
async def disconnect(sid):
    ...
    if origin_url is None:
        print('[ERROR] origin_url is unknown, could not disconnect data subscription with socketid: ', sid)
        return
